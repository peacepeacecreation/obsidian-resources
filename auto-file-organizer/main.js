...
var TagSuggest = class extends TextInputSuggest {
  getSuggestions(inputStr) {
    ...
    for (const file of allFiles) {
      ...
      if (!metadata)
        continue;
      const tags = (0, import_obsidian2.getAllTags)(metadata);
      if (tags) {
        tags.forEach((tag) => {
...
// src/setting.ts
var AutoFileOrganizerSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  async display() {
    ...
    containerEl.empty();
    new import_obsidian3.Setting(containerEl).setName("Priority").setDesc("Decide which mapping takes precedence when both are enabled.").addDropdown((dropdown) => {
      dropdown.addOptions({
      ...
      dropdown.onChange(async (value) => {
        ...
        new import_obsidian3.Notice(`Priority set to: ${value === "extension" ? "Extension" : "Tag"}`);
      });
    });
    containerEl.createEl("h3", { text: "Extension-to-Folder Mapping" });
    new import_obsidian3.Setting(containerEl).setName("Enable Extension Mapping").setDesc("Enable or disable extension-to-folder mapping.").addToggle((toggle) => {
      var _a;
    ...
    for (const file of allFiles) {
      ...
      if (!metadata)
        continue;
      const tags = (0, import_obsidian3.getAllTags)(metadata);
      if (tags) {
    ...
    let newExtension = "";
    let newFolder = "";
    new import_obsidian3.Setting(containerEl).setName("Add new extension mapping").setDesc("Add a new extension and target folder").addText(
      ...
      btn.setButtonText("Add").setCta().onClick(async () => {
        if (newExtension && newFolder) {
          if (isValidExtension(newExtension)) {
            ...
            new import_obsidian3.Notice("The input is invalid.");
          }
        }
      });
    });
    ...
    for (const [extension, folder] of Object.entries(this.plugin.settings.extensionMapping)) {
      new import_obsidian3.Setting(collapsibleSection1).setName(`Extension: ${extension}`).setDesc("Change the folder for this extension").addDropdown((dropdown) => {
        ...
        dropdown.onChange(async (value) => {
          if (value) {
            ...
            await this.plugin.saveSettings();
            new import_obsidian3.Notice(`Folder for .${extension} files updated to: ${value}`);
          }
        });
      }).addButton(
    ...
    new import_obsidian3.Setting(containerEl).setName("Get extension mapping").setDesc("Scan the extension in the file and make mapping extension to folder automatically").addButton((btn) => {
    ...
    let eblackList = "";
    new import_obsidian3.Setting(containerEl).setName("Set Black Folder List").setDesc("Indicate what folder is excluded for automatically get extension mapping").addSearch((search) => {
      new FolderSuggest(this.app, search.inputEl);
      search.setPlaceholder("Search folder...").onChange((folder) => {
        eblackList = folder;
      });
    ...
    for (const [folder1, folder2] of Object.entries(this.plugin.settings.extensionBlackList)) {
      new import_obsidian3.Setting(collapsibleSection3).setName(`Exclude "${folder1}"`).setDesc("This folder is excluded when pushing get extension button").addButton(
        (btn) => btn.setButtonText("Delete").setCta().onClick(async () => {
    ...
    new import_obsidian3.Setting(containerEl).setName("Enable Tag Mapping").setDesc("Enable or disable tag-to-folder mapping.").addToggle((toggle) => {
      var _a;
    ...
    let newTag = "";
    let tagFolder = "";
    new import_obsidian3.Setting(containerEl).setName("Add new tag mapping").setDesc("Add a new tag and target folder").addSearch((search) => {
    ...
    for (const [tag, folder] of Object.entries(this.plugin.settings.tagMapping)) {
      new import_obsidian3.Setting(collapsibleSection2).setName(`Tag: ${tag}`).setDesc("Change the folder for this tag").addDropdown((dropdown) => {
        dropdown.addOption("", "Select folder...");
        allFolders.forEach((f) => dropdown.addOption(f.path, f.path));
        dropdown.setValue(folder);
        dropdown.onChange(async (value) => {
          if (value) {
            ...
            await this.plugin.saveSettings();
            new import_obsidian3.Notice(`Folder for ${tag} files updated to: ${value}`);
          }
        });
      }).addButton(
        (btn) => btn.setButtonText("Delete").setCta().onClick(async () => {
    ...
    new import_obsidian3.Setting(containerEl).setName("Get tag mapping").setDesc("Scan the tag in the file and make mapping tag to folder automatically").addButton((btn) => {
      btn.setButtonText("Start scan").setCta().onClick(async () => {
    ...
    new import_obsidian3.Setting(containerEl).setName("Set Black Folder List").setDesc("Indicate what folder is excluded for automatically get tag mapping").addSearch((search) => {
      new FolderSuggest(this.app, search.inputEl);
      search.setPlaceholder("Search folder...").onChange((folder) => {
        blackList = folder;
      });
    ...
    for (const [folder1, folder2] of Object.entries(this.plugin.settings.tagBlackList)) {
      new import_obsidian3.Setting(collapsibleSection4).setName(`Exclude "${folder1}"`).setDesc("This folder is excluded when pushing get tag mapping button").addButton(
        (btn) => btn.setButtonText("Delete").setCta().onClick(async () => {
    ...
    // Exclusion Folders Section
    containerEl.createEl("h3", { text: "Exclusion Folders" });
    let exclusionFolder = "";
    new import_obsidian3.Setting(containerEl)
      .setName("Add exclusion folder")
      .setDesc("Add a folder to exclude from auto-organization. Files in these folders will not be moved.")
      .addSearch((search) => {
        new FolderSuggest(this.app, search.inputEl);
        search.setPlaceholder("Search folder...").onChange((folder) => {
          exclusionFolder = folder;
        });
      })
      .addButton((btn) => {
        btn.setButtonText("Add").setCta().onClick(async () => {
          if (exclusionFolder && !this.plugin.settings.exclusionFolders.includes(exclusionFolder)) {
            this.plugin.settings.exclusionFolders.push(exclusionFolder);
            await this.plugin.saveSettings();
            this.display();
          } else if (this.plugin.settings.exclusionFolders.includes(exclusionFolder)) {
            new import_obsidian3.Notice("This folder is already in the exclusion list.");
          } else {
            new import_obsidian3.Notice("Please enter a valid folder name.");
          }
        });
      });
    
    // Display existing exclusion folders
    const exclusionSection = containerEl.createEl("details", { attr: { open: "true" } });
    const exclusionSummary = exclusionSection.createEl("summary", { text: "Excluded Folders" });
    exclusionSummary.style.fontSize = "1.2em";
    exclusionSummary.style.margin = "8px";
    exclusionSummary.style.cursor = "pointer";
    
    for (const folder of this.plugin.settings.exclusionFolders) {
      new import_obsidian3.Setting(exclusionSection)
        .setName(`Exclude "${folder}"`)
        .setDesc("Files in this folder will not be auto-organized")
        .addButton((btn) => 
          btn.setButtonText("Remove").setCta().onClick(async () => {
            const index = this.plugin.settings.exclusionFolders.indexOf(folder);
            if (index > -1) {
              this.plugin.settings.exclusionFolders.splice(index, 1);
              await this.plugin.saveSettings();
              this.display();
            }
          })
...
var AutoFileOrganizer = class extends import_obsidian4.Plugin {
  async onload() {
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        new import_obsidian4.Notice("Files organized");
      }
    });
  }
  
  isFileInExclusionFolder(filePath) {
    if (!this.settings.exclusionFolders || this.settings.exclusionFolders.length === 0) {
      return false;
    }
    
    for (const exclusionFolder of this.settings.exclusionFolders) {
      // Normalize paths by removing trailing slashes
      const normalizedExclusionFolder = exclusionFolder.replace(/\/$/, '');
      const normalizedFilePath = filePath.replace(/\/$/, '');
      
      // Check if file is in the exclusion folder or its subfolders
      if (normalizedFilePath.startsWith(normalizedExclusionFolder + '/') || 
          normalizedFilePath === normalizedExclusionFolder) {
        return true;
      }
    }
  ...
  async handleFile(file) {
    ...
    // Check if file is in exclusion folders
    if (this.isFileInExclusionFolder(file.path)) {
      console.log(`File ${file.path} is in exclusion folder, skipping organization.`);
      return null;
    }
  ...
  async organizeVault() {
    ...
    if (movedFiles.length > 0) {
      new import_obsidian4.Notice(`Moved ${movedFiles.length} files:
${movedFiles.join(", ")}`);
    } else {
      new import_obsidian4.Notice("No files were moved.");
    }
  }
  async updateExtensionMappingFromExistingFiles() {
    ...
    new import_obsidian4.Notice(`update extension-to-folder mapping`);
  }
  async updateTagMappingFromExistingFiles() {
    ...
    await this.saveSettings();
    new import_obsidian4.Notice(`update tag-to-folder mapping.`);
  }
};
